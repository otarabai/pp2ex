    ...
    for seqid, seqstring in getnexttestseq(open(targetsfile)):
        ...
        if totalScores is None:
            ...
            for i in range(0, len(totalScores)):
                totalScores[i]['fvalue'] += scores[i]['fvalue']
                totalScores[i]['precision'] += scores[i]['precision']
                totalScores[i]['recall'] += scores[i]['recall']
    
    for sc in totalScores:
        sc['fvalue'] /= counter
        sc['precision'] /= counter
        sc['recall'] /= counter
    print '\n1 Fold:\n'
    for s in totalScores:
        print '%.4f\t%.4f' % (s['recall'], s['precision'])
    return totalScores
...
def main(argv):
    ...
    for indexKey in evaluationIndex:
        ...
        scores = evaluate('kfold/train.fasta', 'kfold/eval.fasta')
